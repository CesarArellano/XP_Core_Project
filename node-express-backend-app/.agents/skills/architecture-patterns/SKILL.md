...
describe('CreateUserUseCase', () => {
  it('returns a successful response with the saved user when the email is unused', async () => {
    // Arrange
    const repo = new InMemoryUserRepository();
    const useCase = new CreateUserUseCase(repo);

    // Act
    const response = await useCase.execute({ email: 'alice@example.com', name: 'Alice' });
  ...
  it('rejects the request with an "already exists" error when the email is already registered', async () => {
    // Arrange
    const repo = new InMemoryUserRepository();
    const useCase = new CreateUserUseCase(repo);
    ...
    // Act
    const response = await useCase.execute({ email: 'alice@example.com', name: 'Alice 2' });
...
Validate invariants in the constructor so an invalid `Email` or `Money` cannot be constructed at all — throw before assigning any field. This surfaces bad data at the boundary, not deep inside business logic.
